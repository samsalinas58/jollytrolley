<template>
    <md-card>
      ...
      <div class="trolley-info-nugget">The next trolley will arrive in &nbsp; <div class="next-trolley callout-text"> {{minutes}} </div> &nbsp;  minutes.</div>
    </md-card>
</template>
    ...
    axios
      ...
      .then(response => {
        ...
        for(const key of keys) {
          ...
          if(key.length == 3){
            this.count++;
            this.trolleys[key] = response.data[key];
          }
        }
        calculateClosestTrolleyTime(this.trolleys);
      });
  },
}
...
function calculateClosestTrolleyTime(trolleys){
  const keys = Object.keys(trolleys)
  for(const key of keys) {
    calculateTrolleyTime(trolleys[key]);
  }
}
...
// Given the coordinate of a Trolley, calculate the amount of minutes away the trolley is
function calculateTrolleyTime(coordinateArray) {
  // Trolley Coordinates
  var start = new google.maps.LatLng({lat: coordinateArray[0], lng: coordinateArray[1]});
